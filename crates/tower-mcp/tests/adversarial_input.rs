...
//!
//! Every test here runs by default. The findings from the original pass
//! landed as `#[ignore]`d reproductions and have since been fixed, so each
//! one now guards a regression rather than describing a bug; the issue number
//! stays in the doc comment so what it guards stays legible.
