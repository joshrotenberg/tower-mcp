...
| Wire revision | Status in tower-mcp 0.20 | Compile-time switch | Lifecycle |
|---|---|---|---|
...
server's `supported` list that use the discover lifecycle, never a legacy
`initialize`-lifecycle version, even one the client would otherwise accept. If
the server's `supported` list has no discover-lifecycle version the client also
enables, `discover` fails with an error that says to fall back to
`initialize`. It does not automatically convert a modern
